document.addEventListener("DOMContentLoaded", () => {
    ...
    const enviarPedidoWhatsApp = () => {
        ...
        const addLoc =
            localStorage.getItem("addLoc") || "Nenhuma instrução adicional";
        const observacoes =
            localStorage.getItem("observacoes") || "Sem observações";
        ...
        Object.entries(bordasSelecionadas).forEach(([nomeBorda, detalhes]) => {
            if (detalhes.quantidade > 0) {
                const nomeLimpo = nomeBorda.replace("-", ""); // Remove o hífen do final do nome
                mensagem += `${nomeLimpo}: ${detalhes.quantidade}x\n`;
            }
        });
        ...
        const linkWhatsApp = `https://api.whatsapp.com/send?phone=5551995694663&text=${encodeURIComponent(
            mensagem
        )}`;
    ...
    function atualizarTotal() {
        ...
        const total = valorProdutos + valorFrete;
        document.getElementById("finalTotal").textContent = `R$${total.toFixed(
            2
        )}`;
    }
    ...
    function enviarDadosParaSheetmonkey() {
        ...
        sabores.forEach((sabor) => {
            saboresString += `Sabor: ${sabor.sabor}\nQuantidade: ${sabor.quantidade}\n`;
        });

        Object.entries(adicoesSelecionadas).forEach(
            ([nomeAdicao, detalhes]) => {
                if (detalhes.quantidade > 0) {
                    const nomeLimpo = nomeAdicao.replace("-", ""); // Remove o hífen do final do nome
                    if (nomeAdicao.toLowerCase().includes("borda")) {
                        bordasString += `Borda: ${nomeLimpo}\nQuantidade: ${detalhes.quantidade}\n`;
                    } else if (nomeAdicao.toLowerCase().includes("molho")) {
                        molhosString += `Molho: ${nomeLimpo}\nQuantidade: ${detalhes.quantidade}\n`;
                    } else {
                        adicionaisString += `Adicional: ${nomeLimpo}\nQuantidade: ${detalhes.quantidade}\n`;
                    }
                }
            }
        );
